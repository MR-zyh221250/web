...
docker run -d --name "$name-api" --network "$name" --memory 256m -e DB_PASSWORD=TestDatabase123 -e PUBLIC_ORIGIN=http://test.local -e MERCHANT_ORIGIN=http://merchant.test -e COOKIE_SECURE=false "$NEON_API_IMAGE" >/dev/null
i=0
...
docker exec "$name-api" node commerce-test.mjs
